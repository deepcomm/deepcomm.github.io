---
layout:     post
title:      Augmentation on Min-Sum Decoders using Model-based ML
...
A $(N,K)$ block code maps a message of length $K$  to a codeword of length $N$ and is uniquely described by its parity check matrix $\mathbb{H}$ of dimensions $(N-K) \times N$, where the rate of the code is $R=\sfrac{K}{N}$. The linear block code can be also represented using a bipartite graph, known as Tanner graph, which can be constructed using its parity check matrix $\mathbb{H}$. The Tanner graph consists of two types of nodes. We refer to them as Check Nodes (CN) that represent the parity check equations and Variable Nodes (VN) that represent the symbols in codeword. There is an edge present between a check node $c$ and variable node $v$ if $\mathbb{H}(c,v) = 1$.
...
# Create a Linear Block code object : BCH, Block_length = 63
block_len = 4063
linear_code = deepcommpy.nams.LinearCode(code='bch', block_len = block_len)
...
for snr in snr_range:
    ...
    ber_max = torch.ne(message_bits, decoded).float().mean().item()
```
...
[Interpreting Neural Min-Sum Decoders](https://arxiv.org/abs/2209.15614), Sravan Kumar Ankireddy, Hyeji Kim. International Conference on Communications (ICC), 2023.
