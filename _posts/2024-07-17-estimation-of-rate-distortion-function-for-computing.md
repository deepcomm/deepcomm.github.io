...
In the ever-evolving landscape of data science, the quest for efficient data compression methods remains a critical challenge. Compression not only saves storage space but also enhances data transmission efficiency, crucial for everything from streaming video to cloud storage. But how do we know when a compression method is truly efficient? The answer lies in understanding the rate-distortion function, a concept that benchmarks the trade-off between compression rate and the distortion of the compressed data.

### Rate-Distortion Function
The rate-distortion function is a fundamental concept in information theory, representing the minimum data rate required to transmit or store information such that the distortion (or loss of quality) remains below a certain threshold. It serves as a yardstick for the efficiency of compression algorithms.


The rate-distortion function concept can be extended to encompass scenarios where side information, correlated with the input source, is available at the decoder, or at both the encoder and decoder. This adaptation is widely recognized as the Wyner-Ziv rate-distortion function. Additionally, the notion of the rate-distortion is further broadened by considering communication systems where the goal is to compute a function of the source. Such applications of the Wyner-Ziv rate-distortion function are commonly referred to as **Coding for Computing**.


In our work, we are interested in the system model depicted below, where switch (A) remains open and switch (B) is closed. This system model focuses on minimizing the distortion between a target output $Z$ and its estimated counterpart $\hat{Z}$ through the optimized encoder and decoder modules. The encoder receives an input source $X$ and compresses it into a codeword $U$. The decoder, utilizing this codeword along with side information $Y$, produces the estimated output $\hat{Z}$. Notably, this framework permits the target output $Z$ to differ from the input source $X$, allowing $Z$ to be any functional output $g(X,Y)$ tailored to specific system requirements.
...
$R_{\text{D}}(D)=\text{min}_{q_{U|X}(u|x),f(u,y) :\mathbb{E}[d(Z,\hat{Z})]\le{D}}I(X;U|Y)$
...
We begin by reformulating the optimization term as follows by using the convexity property of the rate-distortion function. For a given $s$, we can build a Lagrangian form optimziation problem as follows.
...
If we solve the Lagrangian form, the optimization variables (encoder, decoder) yield a point(s) in the true rate-distortion function.
...
By doing this, now we have all the main components of the optimization problems as optimization variables. We perform gradient-based update where the weights are updated in disrection of minimizing (3). 
...
How do we verify the performance of our algorithm? There are some very special cases where the rate-distortion function for computing with side information is known as a **closed-form**.
...
Our objective in thie section is to apply our algorithm to estimate rate-distortion points and assess its accuracy in mapping these points on the true rate-distortion curve!

For the parameterization of the distributions and the decoder, we used 2-layer MLPs. The details of the simulation setup can be found in our original paper. 
...
To enhance compression efficiency, uplink (UL) CSI can be utilized as side information $Y$ (See right hand side of the above image). This is based on the observation that UL CSI is typically acquired (available) via pilot transmissions from the UE to BS, and is correlated with DL CSI due to frequency-invariant characteristics. 

We will use our algorithm to estimate the rate-distortion function with decoder side information for this setting. 
...
The four points plotted on $\hat{R}_{\text{D,C}}$ and $\hat{R}_{\text{C}}$ denote distinct estimated rate-distortion points, with their positions corresponding to specific $s$ values: -100, -10, -1, and -0.1, arranged from left to right. $\hat{R}_{\text{C}}$, the estimated ${R}_{\text{C}}$, is obtained by \cite{yang2021towards} and using the same neural architectures but which ignores the side information. By adjusting $s$ values, we explore distortion levels from -8dB to approximately -23dB, connecting these points linearly to serve as an upper bound for the estimated rate-distortion curves. We also adopt VQ-VAE-based compression algorithm for purpose of comparison. The rate-distortion curves from the constructive algorithm with and without side information (light blue/yellow) are generated by varying the compression bit rates as $l_{\text{cl}}\in \{16, 32, 64, 128\}$ and connecting the points.
