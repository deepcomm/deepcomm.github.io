...
Significant advancements in DNNs have led to developments in millimeter wave wireless communication. However, these deep learning-based methods require extensive training data, which is impractical to collect in real-world environments. For example, training beam alignment algorithms requires around 100K channel measurements for a typical macrocell sector. Existing methods can perform suboptimally if only a limited number of data samples are available at specific locations. Therefore, we suggest a channel data synthesizing method using a conditional diffusion model, which captures the distribution of the site-specific channel and boosts the dataset.
